...
export const ItemDialogEdit = ({ item, showAlert, onOpenChange }: { item: IItem; showAlert: boolean; onOpenChange: (isOpen: boolean) => void }) => {
  ...
  return (
    <AlertDialog open={showAlert} onOpenChange={(isOpen) => onOpenChange(isOpen)}>
      <AlertDialogContent className='bg-topbar'>
        <AlertDialogHeader>
          <AlertDialogTitle>Редактировать Товар: {item.name} ? </AlertDialogTitle>
